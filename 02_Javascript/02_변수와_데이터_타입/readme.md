...
### 1) 메모리
 ...
 어떤 값을 기록함 --> 하드디스크, USB, RAM --> 똑같은 사이즈의 메모리(반도체)를 얼마나 쪼개느냐에 따라 용량이 좌우됨.

 #### 도체
 - 도체 : 전류가 흐름
 - 부도체 : 전류가 흐르지 않음
 - 반도체 : 열이나 전기를 가해서 전기가 흐르거나 흐르지않는 성질을 바꿀 수 있는 물체 --> on->1 off->0

### 2) 2진수
|2진수|... |...|
|...|...|...|
| 1개 | 1bit(2개) | 0,1 |
| 2개 | 2bit(4개) | 00, 01, 10, 11| 
| 3개 | 3bit(8개) | 00, 01, 10, 11....|
| n개 | nbit | 2의 2제곱의 정보 |

- 2진수 개연 알파벳 표현 가능 => ANSI표현법
- ANSI + 부호를 위한 1bit => 영어 +1개국어(2개 표현)
- 컴퓨터 최소용량 1byte = 8bit => 256개로 구성
- 영어, 숫자, 특수문자 => 1byte = 한글자
- 한자,한글 => 2byte = 한글자
- 다국어를 한번에 표현하기 위해 언어를 구분하지 않음 --> 3byte로 한글자를 표현 => UTF-8
- ANSI, 확장ANSI ,UTF-9: 글자를 표현하기 위한 2진수의 조합 방법 -> encoding, 캐릭터셋
- 숫자가 표현할 수 있는 범위를 넘기면 다시 1로 회귀하는 현상 => "overflow" 
- byte가 모여서 어떻게 해석하느냐에 따라 글자, 숫자가 될 수 있음
   - 메모리에 기록된 정도 (2진수 집합) --> 변수(data)
   - 2진수 집합을 몇개씩 묶어서 해석 --> 데이터 타입, 변수의 자료형, data형

**한글 => 2byte, 영어 => 1byte, 숫자(정수) => 4byte(32개), 다국어 => 3byte(24개)**

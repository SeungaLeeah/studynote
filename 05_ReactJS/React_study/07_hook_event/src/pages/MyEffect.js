...
const MyEffect = () => {
  ...
  /** 이 컴포넌트가 화면에 딱 등장함과 동시에 1회 실행됨 */
  React.useEffect(()=>{
    console.clear();
    console.log('[MyEffect] %s ::: 화면에 컴포넌트가 처음 로드될 때 처리되어야 할 기능', new Date());
  ...
  React.useEffect(()=>{
    return () => {
      console.log('[MyEffect3] %s ::: 이 컴포넌트가 화면에서 사라지가 직전에 처리되어야 할 기능', new Date());
    };
  });

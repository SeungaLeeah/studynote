...
class DBPool{
    ...
    static connectionInfo = {
        ...
        database: process.env.DATABASE_SCHEMA,  // 사용하조가 하는 데이터 베이스 이름
    ...
    constructor(){
        ...
        this.pool.on('connection', (connection)=>{
            logger.info(`>> DATABASE 접속된 [threadId=${connection.threadId}]`);

            // 이 객체로 전달되는 SQL수행 기능을 가로챔
        ...
        async getConnection(){
            ...
            try{
                ...
                //임대한 자원이 있다면 반드시 반납 해야함.
                if(dbcon){dbcon.replace();}
                logger.error(err);
                throw err;
            }
            return dbcon;

...
(async()=>{
    ...
    try{
        ...
        console.log(result1[0].affectedRows);   // 저장될 행의 수
        console.log(result1[0].insertId);       // insert id
        ...
        console.group("INSERT 처리결과");
        console.log('저장된 데이터 수: ' + result2.affectedRows);
        console.log('생성된 PK 값: ' + result2.insertId);
        console.groupEnd();

        // 배열을 비구조화 시키고, 그 안에 있는 0번째를 JSON으로 한번 더 비구조 문법을 적용할 수 도 있다.
        ...
        console.log('생성된 PK 값: ' + insertId);
        console.groupEnd();

        // UPDATE절 수행 --> WHERE절의 지정이 중요하다.
        const [result3] = await dbcon.query('UPDATE department SET dname=? WHERE deptno >= ?',
            ["수정학과", firstId]);
            console.group('UPDATE 처리 결과');
            console.log('수정된 데이터 수: ' + result3.affectedRows);
            console.groupEnd();

        const [result4] = await dbcon.query('DELETE FROM department WHERE deptno >= ?',
            [firstId]);
            console.group('DELETE 처리 결과');
            console.log('삭제된 데이터 수: ' + result4.affectedRows);
            console.groupEnd();
        } catch(err){
            console.log(err);
            return;

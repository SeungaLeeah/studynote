...
server.on('connection',(socket)=>{
    // 콜백함수에 전달되는 socket 객체를 사용하여 접속한 클라이어트의 정보를 파악한다.
    logger.debug('프론트엔드가 접속했습니다. : ' + socket.remoteAddress + ',' +  socket.remotePort);
    logger.debug(socket); 
});

/** connection 이벤트 발생 직후 프론트엔드에게 결과 값을 되돌려 주기 위해 호출되는 이벤트 */
// req(request) --> 요청객체: 브라우저가 서버에게 전달하는 정보를 담고있다.
...
/** 서버 종료 이벤트 */
// 정상적인 상황에서는 발생한 가능성이 없다.
server.on('close', function(){
    logger.debug('백엔트가 종료되었습니다.');
});
